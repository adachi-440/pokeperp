...
contract RiskEngine is IRiskEngine {
    ...
    constructor(IVault _vault, IOracleAdapter _oracle, IPerpPositions _perp, uint256 _imr, uint256 _mmr, uint256 _contractSize) {
        owner = msg.sender;
        vault = _vault;
        oracle = _oracle;
        perp = _perp;
        ...
        imr = _imr; mmr = _mmr; contractSize = _contractSize;
    }
    ...
    function setLinks(IVault _vault, IOracleAdapter _oracle, IPerpPositions _perp) external onlyOwner {
        vault = _vault; oracle = _oracle; perp = _perp;
    }
    ...
    function _abs(int256 x) internal pure returns (uint256) { return uint256(x >= 0 ? x : -x); }
}
